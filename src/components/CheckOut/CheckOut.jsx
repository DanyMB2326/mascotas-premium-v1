...
const Field = ({ label, required, error, children }) => (
  <div className={`ch-field${error ? ' ch-field--err' : ''}`}>
    <label className="ch-label">
      {label}{required && <span className="ch-req">*</span>}
    </label>
    {children}
...
const CardSimulator = ({ value, onChange, errors }) => {
  ...
  const handleNum = (e) =>
    onChange({ ...value, number: maskCard(e.target.value) });
    ...
    <div className="ch-card-sim">
      <div className={`card-preview card-preview--${brand.toLowerCase()}`}>
        <div className="cp-top">
          <span className="cp-brand">💳 {brand}</span>
          <span className="cp-chip">▬▬</span>
        </div>
      ...
      <Field label="Número de tarjeta" required error={errors?.number}>
        <input
          value={value.number || ''} onChange={handleNum}
      ...
      <div className="ch-grid-2">
        <Field label="Vencimiento (MM/AA)" required error={errors?.expiry}>
          <input
            value={value.expiry || ''} onChange={handleExp}
            placeholder="08/27" maxLength={5} inputMode="numeric"
            autoComplete="cc-exp"
          />
        </Field>
      ...
      <p className="ch-secure-note">
        🔒 Pago simulado. Ningún dato se procesa ni se almacena.
      </p>
    </div>
  );
};
...
const Checkout = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (form.formaPago === 'tarjeta-nueva') {
      ...
      setProcessing(true);
      await new Promise((r) => setTimeout(r, 2200)); // Simulación
      setProcessing(false);
    }
    ...
    try {
      ...
      // ─────────────────────────────────────────────────────
      // 2. Actualizar contadores en el documento del usuario
      //    para que el dashboard de Clientes muestre datos reales
      ...
      if (user?.uid) {
        try {
          ...
          // No interrumpir el flujo de compra si falla el contador
          console.warn('[Checkout] No se pudieron actualizar los contadores del usuario.');
        }
      }
  ...
  if (success) return (
    <div className="ch-success">
      ...
      <p>
        Nos pondremos en contacto contigo en las próximas horas para
        coordinar la entrega. Revisa tu WhatsApp.
      </p>
  ...
  const total = totalPrice + envio;
  const selectedCard =
    (profile.cards ?? []).find((c) => c.isPrimary) ??
    profile.cards?.[0] ??
    null;
      ...
      <div className="ch-layout">
        <div className="ch-form-col">
          ...
          <section className="ch-section">
            <h2 className="ch-section-title">
              <span className="ch-step">1</span> Datos de contacto
            </h2>
            <div className="ch-grid-2">
            ...
            <div className="ch-grid-2">
              ...
              <Field label="Correo electrónico" required error={errors.email}>
                <input
                  ...
                  placeholder="hola@correo.com" type="email"
                />
              </Field>
            </div>
          </section>
          ...
          <section className="ch-section">
            <h2 className="ch-section-title">
              <span className="ch-step">2</span> Dirección de entrega
            </h2>
            ...
            <div className="ch-grid-2">
              ...
              <Field label="Código postal" required error={errors.cp}>
                <input
                  ...
                  placeholder="06600" maxLength={5} type="number"
                />
              </Field>
            </div>
            ...
            <div className="ch-cdmx-notice">
              📍 Solo realizamos entregas en CDMX por ahora.
            </div>
          </section>
          ...
          <section className="ch-section">
            <h2 className="ch-section-title">
              <span className="ch-step">3</span> Forma de pago
            </h2>

            <div className="ch-pago-grid">
              {profile.cards.length > 0 && (
                <label
                  className={`ch-pago-opt${form.formaPago === 'tarjeta-guardada' ? ' ch-pago-opt--active' : ''}`}
                >
                  <input
                    type="radio" name="fp" value="tarjeta-guardada"
                    checked={form.formaPago === 'tarjeta-guardada'}
                    onChange={set('formaPago')} className="ch-radio"
                  />
                  <span>💳</span>
              ...
              {FORMAS_PAGO
                .filter((fp) => fp.id !== 'tarjeta-guardada')
                .map((fp) => (
                  <label
                    key={fp.id}
                    className={`ch-pago-opt${form.formaPago === fp.id ? ' ch-pago-opt--active' : ''}`}
                  >
                    <input
                      type="radio" name="fp" value={fp.id}
                      checked={form.formaPago === fp.id}
                      onChange={set('formaPago')} className="ch-radio"
                    />
                    <span>{fp.icon}</span>
            ...
            {form.formaPago === 'tarjeta-guardada' && selectedCard && (
              <div className="ch-saved-card-preview">
                <div
                  className={`card-preview card-preview--${selectedCard.brand.toLowerCase()}`}
                  style={{ padding: '1.1rem 1.5rem', minHeight: 'auto' }}
                >
                  <div className="cp-top">
                    <span className="cp-brand">💳 {selectedCard.brand}</span>
                    <span className="cp-chip">▬▬</span>
                  </div>
                  <div className="cp-number">···· ···· ···· {selectedCard.last4}</div>
                  <div className="cp-bottom">
                    <div>
                      <span className="cp-lbl">Alias</span>
                      <span className="cp-val">{selectedCard.alias}</span>
                    </div>
                    <div>
                      <span className="cp-lbl">Vence</span>
                      <span className="cp-val">{selectedCard.expiry}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {form.formaPago === 'transferencia' && (
              <div className="ch-pago-detail">
                📋 Compartiremos los datos bancarios por WhatsApp una vez confirmado el pedido.
              </div>
            )}
            {form.formaPago === 'efectivo' && (
              <div className="ch-pago-detail">
                💵 Ten el monto exacto listo al momento de la entrega.
              </div>
            )}
          </section>
        </div>
        ...
        <aside className="ch-summary">
          ...
          <div className="ch-sum-items">
            {cartItems.map((item) => (
              <div key={item.id} className="ch-sum-item">
                ...
                <span className="ch-sum-item-price">
          ...
          <div className="ch-sum-lines">
            ...
            <div className="ch-sum-line">
              <span>Envío</span>
              <span className={envio === 0 ? 'ch-free' : ''}>
                {envio === 0 ? '¡Gratis! 🎉' : `$${envio} MXN`}
              </span>
            </div>
            {envio > 0 && (
              <p className="ch-envio-hint">
                Agrega ${(700 - totalPrice).toLocaleString('es-MX')} MXN más para envío gratis
              </p>
            )}
          </div>
          <div className="ch-sum-divider" />
          <div className="ch-sum-total">
            <span>Total</span>
            <span>${total.toLocaleString('es-MX')} MXN</span>
          </div>
          <button
            className="ch-confirm-btn"
            onClick={handleSubmit}
            disabled={loading}
          >
            {loading
              ? <span className="ch-loading">Procesando<span className="ch-dots">…</span></span>
              : '✓ Confirmar pedido'
            }
          </button>
